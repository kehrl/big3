# Make some plots for my General 2016 presentation
...
ax.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_overviewmap.pdf"),format='PDF',transparent=True,dpi=600)
plt.close()

del m,ax,cs,cb,clip,xcoast,ycoast,nx,ny,fig,xvel,yvel,vel,coast,velgrid
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_Helheim_map.pdf"),FORMAT='PDF',dpi=600,transparent=True)
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_Helheim_map1.pdf"),FORMAT='PDF',dpi=600,transparent=True)
plt.close()

del ax,patch,path,xmin,xmax,ymin,ymax,cbaxes,cb,vel_masked,cx,xmask,ymask,mask,xvel,yvel,vel,ximage,yimage,image,imagetime,fig
...
plt.subplots_adjust(wspace=0,hspace=0,top=0.98,right=0.98,left=0.02,bottom=0.02)
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_Kanger_map.pdf"),FORMAT='PDF',dpi=600,transparent=True)
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_Kanger_map1.pdf"),FORMAT='PDF',dpi=600,transparent=True)
plt.close()
...
for child in ax.get_children():
  if isinstance(child, matplotlib.spines.Spine):
    child.set_color(textcolor)
ax.tick_params(axis='both',colors=textcolor,which='both')
ax.arrow(2007,3.3,0,0.6,head_width=0.1,fc=textcolor,ec=textcolor,lw=1.5,head_length=0.1)
ax.text(2007.3,3.3,'Advance',fontsize=16,color=textcolor)
ax.arrow(2007,-2.7,0,-0.6,head_width=0.1,fc=textcolor,ec=textcolor,lw=1.5,head_length=0.1)
ax.text(2007.3,-3.3,'Retreat',fontsize=16,color=textcolor)
ax.tick_params(axis='both',colors=textcolor,which='both')
...
plt.ylabel('Terminus \n (km)',color=textcolor,fontsize=14)
...
plt.ylabel('Glacier speed \n (km/yr)',color=textcolor,fontsize=14)
plt.yticks(np.arange(4,14,2),fontsize=12)
plt.ylim([4.5,13])
plt.xticks(np.arange(2000,2020,5))
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
...
plt.ylabel('Surface elevation \n (m asl)',color=textcolor,fontsize=14)
plt.ylim([90,350])
plt.yticks(np.arange(80,360,40),fontsize=12)
plt.xticks(np.arange(2000,2020,5))
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
...
plt.yticks(np.arange(5,14,1),fontsize=12)
plt.ylim([4.5,13])
plt.xticks(np.arange(2000,2020,5))
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
...
plt.tight_layout()
plt.subplots_adjust(wspace=0.12,hspace=0.04,top=0.98,right=0.99,left=0.095,bottom=0.05)
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_timeseries_2000.pdf"),format="PDF")
plt.close()
...
for glacier in ['Helheim','Kanger']:
  ...
  plt.savefig(os.path.join(os.getenv("HOME"),'Bigtmp/General_'+glacier+'_zs_flowline.pdf'),FORMAT='PDF',dpi=600)
  plt.close()
...
for glacier in ['Helheim','Kanger']:
  ...
  for k in range(0,5+len(demtimes)):
    ...
    if k > 4: 
      ...
      if glacier == 'Helheim':
        time1 = 2008.0; time2 = 2016.5; 
      elif glacier == 'Kanger':
        time1 = 2008.0; time2 = 2016.
    ...
    if vbars == 'runoff':
      for i in range(0,len(year_runoff)):
        ...
        ax.add_patch(patch)
    if (plot_calving) and ((k == 4) or (glacier == 'Kanger')):
    ...
    ax.tick_params('both', length=3, width=1, which='minor')
    if k > 4:
      demtime = datelib.date_to_fracyear(int(demtimes[k-5][0:4]),int(demtimes[k-5][4:6]),int(demtimes[k-5][6:8]))
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_"+glacier+"_all"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
  ...
  for k in range(0,len(demtimes)):
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_"+glacier+"_dem"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
  ...
  for k in range(0,len(demtimes)):   
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_"+glacier+"_image"+str(k)+".pdf"),FORMAT='PDF',dpi=600,transparent=True)
    plt.close()
...
for glacier in ['Helheim','Kanger']:
  ...
  for k in range(0,2):
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_"+glacier+"_icefront"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
...
for glacier in ['Helheim','Kanger']:
  ...
  plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_"+glacier+"_flux.pdf"),FORMAT='PDF',dpi=600)
  plt.close()

del inttime, time, Q, fig, ax, labels, x_formatter, colorpt, meanQ

######################################
# Ice-front position vs. environment #
######################################
...
for glacier in ['Helheim','Kanger']:
  ...
  for k in range(0,2):
    ...
    # Plot SIF
    ax = plt.subplot(gs[2, :]) 
    plt.plot(timesif,sif,lw=1.5,color='r')
    x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
    ax.xaxis.set_major_formatter(x_formatter)
    ax.xaxis.set_minor_locator(AutoMinorLocator(2))
    ax.yaxis.set_minor_locator(AutoMinorLocator(2))
    ...
    plt.ylim([0,1])
    plt.yticks([0,1.1,0.5])
    plt.ylabel('Sea ice \n fraction',fontsize=12,fontname='Arial')
    for i in range(0,len(yH_runoff)):
      path = matplotlib.path.Path([[day1_runoff[i],-165],[day1_runoff[i],160],[day2_runoff[i],160],[day2_runoff[i],-165]])
      patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
      ax.add_patch(patch) 
    ...
    plt.subplot(gs[-2,:])
    ax = plt.gca()
    plt.plot(timesst,sst-273.15,lw=1.5,color='g')
    x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
    ax.xaxis.set_major_formatter(x_formatter)
    ax.xaxis.set_minor_locator(AutoMinorLocator(2))
    ax.yaxis.set_minor_locator(AutoMinorLocator(2))
    ...
    plt.yticks([-2,0,2,4])
    plt.ylim([-3,6])
    plt.ylabel('SST \n (deg C)',fontsize=12,fontname='Arial')
    for i in range(0,len(yH_runoff)):
      path = matplotlib.path.Path([[day1_runoff[i],-165],[day1_runoff[i],160],[day2_runoff[i],160],[day2_runoff[i],-165]])
      patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
      ax.add_patch(patch) 
    ...
    # Plot surface runoff
    plt.subplot(gs[-1:,:])
    plt.plot(timerac,runrac,'b',lw=1.5)
    ax = plt.gca()
    plt.ylabel('Runoff \n'+r'(kg/m$^2$yr)',fontname='Arial',fontsize=12)
    plt.xticks(range(2000,2017))
    labels=[]
    for i in range(2000,2017):
      labels.append('Jan \n'+str(i))
    ...
    for i in ind:
      ...
      if glacier == 'Helheim':
        ax.text(year_runoff[i]+0.62,48,'{0:.0f}'.format(np.round(total_runoff[i],decimals=-1)),fontsize=10,fontname='Arial')
        if i == ind[0]:
          ax.text(year_runoff[i]+0.62,37,r'kg/m$^{2}$',fontsize=10,fontname='Arial')
      elif glacier == 'Kanger':
        if i == ind[0]:
          ax.text(year_runoff[i]+0.58,48,'{0:.0f}'.format(np.round(total_runoff[i],decimals=-1)),fontsize=10,fontname='Arial')
          ax.text(year_runoff[i]+0.58,37,r'kg/m$^{2}$',fontsize=10,fontname='Arial')
        else:
          ax.text(year_runoff[i]+0.59,48,'{0:.0f}'.format(np.round(total_runoff[i],decimals=-1)),fontsize=10,fontname='Arial')

    plt.tight_layout()
    plt.subplots_adjust(hspace=0.03,wspace=0,top=0.98,right=0.97,left=0.1,bottom=0.1) 
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_"+glacier+"_icefront"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
...
#########################
# Terminus driven model #
#########################

plt.figure(figsize=(5,2))
ax = plt.gca()
plt.plot(terminus_time_H,terminus_val_H/1e3,'ko',markersize=3)
plt.xticks(range(2000,2017))
labels=[]
for i in range(2000,2017):
  labels.append('Jan \n'+str(i))
...
ax.tick_params('both', length=3, width=1, which='minor')
ax.set_xticklabels(labels,fontsize=12,fontname='Arial')
plt.ylabel('Terminus (km)',fontsize=12)
plt.tight_layout()
plt.subplots_adjust(hspace=0.03,wspace=0,top=0.98,right=0.95,left=0.12,bottom=0.2) 
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_terminusdriven.pdf"),FORMAT='PDF',dpi=600)
plt.close()
...
plt.tight_layout()
plt.subplots_adjust(hspace=0.03,wspace=0,top=0.98,right=0.95,left=0.12,bottom=0.2) 
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_icebergcalvingmodel.pdf"),FORMAT='PDF',dpi=600)
plt.close()

##########################
# Example for inversions #
##########################

inversetimes = [2001,2003,2004,2005,2006,2011.21,2011.45,\
                2011.65,2011.87,2012.20,2012.48,2012.69,2012.93,\
                2013.11,2013.35,2013.59,2013.83,2014.07,2014.35,2014.57,\
                2014.79]
...
plt.xlim([2000,2016])
plt.xticks(np.arange(2000,2020,5))
for i in range(0,len(inversetimes)):
  plt.plot([inversetimes[i],inversetimes[i]],[-5,5],'k',lw=2)
plt.plot(terminus_time_H,terminus_val_H/1e3,'ro',markersize=4)
plt.ylabel('Terminus \n (km)',color=textcolor,fontsize=14)
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
...
plt.xlim([2000,2016])
for i in range(0,len(inversetimes)):
  plt.plot([inversetimes[i],inversetimes[i]],[4,14],'k',lw=2)
plt.plot(vel_time_H,vel_val_H[:,1]/1e3,'ro',markersize=4)
plt.ylabel('Glacier speed \n (km/yr)',color=textcolor,fontsize=14)
plt.yticks(np.arange(4,14,2),fontsize=12)
plt.ylim([4.5,13])
...
plt.xlim([2000,2016])
for i in range(0,len(inversetimes)):
  plt.plot([inversetimes[i],inversetimes[i]],[80,350],'k',lw=2)
plt.plot(time_atm_H,zpt_atm_H[:,1],'ro',markersize=4)
plt.plot(time_dem_H,zpt_dem_H[:,1],'ro',markersize=4)
plt.ylabel('Surface elevation \n (m asl)',color=textcolor,fontsize=14)
plt.yticks(np.arange(80,360,40),fontsize=12)
plt.ylim([90,350])
plt.xticks(np.arange(2000,2020,5),fontsize=14)
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)

plt.tight_layout()
plt.subplots_adjust(hspace=0.03,wspace=0,top=0.98,right=0.95,left=0.17,bottom=0.05) 
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/General_inversions.pdf"),FORMAT='PDF',dpi=600)
plt.close()