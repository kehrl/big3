# Make some plots for my IGS 2016 presentation
...
ax.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_overviewmap.pdf"),format='PDF',transparent=True,dpi=600)
plt.close()
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Helheim_map.pdf"),FORMAT='PDF',dpi=600,transparent=True)
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Helheim_map1.pdf"),FORMAT='PDF',dpi=600,transparent=True)
plt.close()
...
plt.subplots_adjust(wspace=0,hspace=0,top=0.98,right=0.98,left=0.02,bottom=0.02)
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Kanger_map.pdf"),FORMAT='PDF',dpi=600,transparent=True)
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Kanger_map1.pdf"),FORMAT='PDF',dpi=600,transparent=True)
plt.close()
...
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Helheim_2000.pdf"),format="PDF")
plt.close()
...
plt.xlim([2000,2016])
plt.xticks(np.arange(2000,2020,4))
plt.plot(terminus_time_K,terminus_val_K/1e3,'bo',markersize=3.5)
plt.ylabel('Terminus \n (km)',color='k',fontsize=14)
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
ax.set_xticklabels([])
...
ax.tick_params(axis='both',colors='k',which='both')
plt.xlim([2000,2016])
plt.plot(vel_time_K,vel_val_K[:,1]/1e3,'bo',markersize=3.5)
plt.ylabel('Glacier speed \n (km/yr)',color='k',fontsize=14)
...
ax.tick_params(axis='both',colors='k',which='both')
plt.xlim([2000,2016])
plt.plot(time_atm_K,zpt_atm_K[:,1],'bo',markersize=3.5)
plt.plot(time_dem_K,zpt_dem_K[:,1],'bo',markersize=3.5)
plt.ylabel('Surface elevation \n (m asl)',color='k',fontsize=14)
#plt.ylim([4.5,12.5])
plt.yticks(np.arange(120,360,40),fontsize=12)
plt.ylim([90,350])
plt.xticks(np.arange(2000,2020,4))
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
...
plt.tight_layout()
plt.subplots_adjust(wspace=0.04,hspace=0.04,top=0.98,right=0.95,left=0.2,bottom=0.05)
plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Kanger_2000.pdf"),format="PDF")
plt.close()
...
for glacier in ['Helheim','Kanger']:
  ...
  plt.savefig(os.path.join(os.getenv("HOME"),'Bigtmp/IGS_'+glacier+'_zs_flowline.pdf'),FORMAT='PDF',dpi=600)
  plt.close()
...
for glacier in ['Helheim','Kanger']:
  ...
  for k in range(0,5+len(demtimes)):
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_"+glacier+"_all"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
  ...
  for k in range(0,len(demtimes)):
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_"+glacier+"_dem"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_"+glacier+"_image"+str(k)+".pdf"),FORMAT='PDF',dpi=600,transparent=True)
    plt.close()
  
  del cresis,dcresis,zcresis

#############################
# Ice fronts vs environment #
#############################
...
for glacier in ['Helheim','Kanger']:
  ...
  for k in range(0,2):
    ...
    plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_"+glacier+"_icefront"+str(k)+".pdf"),FORMAT='PDF',dpi=600)
    plt.close()
...
for i in range(0,2):
  ...
  plt.savefig(os.path.join(os.getenv("HOME"),"Bigtmp/IGS_Helheim_unground"+str(i)+".pdf"),FORMAT='PDF',dpi=600,transparent=True)
  plt.close()
