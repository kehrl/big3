...
FUNCTION XCalvingFrontIni ( Model, nodenumber, x) RESULT(CalvingFront)
   ...
   LOGICAL :: FirstTimeCalving=.True.

   SAVE FirstTimeCalving
   SAVE Zs0 
   ...
   IF (FirstTimeCalving) THEN
        FirstTimeCalving = .False.
...
! Find the calving front if the glacier is advancing
FUNCTION Advance ( Model, nodenumber, dumy) RESULT(XTotDelta)
   	...
   	REAL(KIND=dp) :: minvelcalve, XTotDelta, XDelta, dt, CalvingFront, dumy     
	INTEGER :: N, BoundaryNodes, minvelcalveNode, nodenumber,  i, dim, Timestep, LastTimestep
	LOGICAL :: FirstTimeAdvance = .TRUE.

   	SAVE FirstTimeAdvance 
	...
	IF( Timestep > LastTimestep) THEN
	    ...
	    print *,'USF_Calving: Terminus position changed by',XtotDelta
...
!InterpolateMeshToMesh( OldMesh, NewMesh, OldVariables, &
...
! Find the calving front if the glacier is advancing
FUNCTION Retreat (Model, nodenumber, dumy) RESULT(XTotDelta)
   	USE types
   	USE CoordinateSystems
   	USE SolverUtils
   	USE ElementDescription
