...
!******************************************************************************
  ...
  TYPE(Variable_t), POINTER :: PointerToVariable, bedrockVar, MeshUpdateVar
  TYPE(Nodes_t), SAVE :: Nodes

  LOGICAL :: AllocationsDone = .FALSE., GotIt, stat

  INTEGER :: i, mn, n, t, Nn, istat, DIM, MSum, ZSum, bedrockSource
  INTEGER, POINTER :: Permutation(:), bedrockPerm(:), MeshUpdatePerm(:)

  REAL(KIND=dp), POINTER :: VariableValues(:), MeshUpdateValues(:)
  REAL(KIND=dp) :: z, toler, zdelta, x
...
!  NULLIFY(bedrockPerm,bedrockVar)
  ...
  DO t = 1, Solver % NumberOfActiveElements
     ...
     DO i = 1, n
        ...
        IF (Nn==0) CYCLE
        IF (DIM == 2) THEN
          MeshUpdateVar => VariableGet( Model % Variables, 'Mesh Update 2' )
    	  MeshUpdatePerm    => MeshUpdateVar % Perm
    	  MeshUpdateValues  => MeshUpdateVar % Values
          zdelta = MeshUpdateValues(MeshUpdatePerm(i))
          z = Nodes % y( i )
          z = z + zdelta
        ELSE IF (DIM == 3) THEN
           z = Nodes % z( i )
