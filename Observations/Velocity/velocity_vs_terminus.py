...
dists_eul = -1.0*np.array([1.0,5.0,10.0,20.0,30.0]) # kilometers
dists_mel = np.array([0.5,1.0,2.0,5.0])
...
ind_mel=[]
for i in range(0,len(dists_mel)):
  ind_mel.append( (abs(dists - dists_mel[i]*1e3)).argmin() )

# Load velocities for glacier and ice melange
veleul_val,veleul_time,veleul_error = velocity.velocity_at_eulpoints(x[ind_eul],y[ind_eul],glacier)
...
# Chop to desired time interval
indt = np.where((veleul_time > time1) & (veleul_time < time2))[0]
veleul_time = veleul_time[indt]
veleul_val = veleul_val[indt,:]
...
plt.plot(rift_time,(rift_val)/1e3,'ro',linewidth=2,markersize=3)
ax = plt.gca()
...
for i in range(0,len(dists_mel)):
  ...
  plt.plot(velmel_time[nonnan],(velmel_val[nonnan,i]-veleul_val[nonnan,0])/1e3,'^',color=coloptions[i],label=str(dists_mel[i])+' km',linewidth=1,markersize=3)
plt.ylabel('Melange speed \n (km/yr)',fontsize=8,fontname="Arial")
ax = plt.gca()
plt.yticks(np.arange(-6,6,2),fontsize=8,fontname="Arial")
plt.ylim([-4,4])
x_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
ax.xaxis.set_major_formatter(x_formatter)
ax.xaxis.set_minor_locator(AutoMinorLocator(2))
...
for k in range(0,len(years)):
  ...
  for i in range(0,len(ind)):
    ...
    if np.sqrt((x[j]-bedpts[cind,0])**2+(y[j]-bedpts[cind,1])**2) < 500:
...
ax2.plot(terminus_val/1e3,terminus_time,'.-',color='0.7')
