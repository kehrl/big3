...
plt.plot(time_H[ind],Q_H[ind,0]*917./1e12,'b.',markersize=4,label = 'Flux in')
ind = np.where(~(np.isnan(Q_H[:,1])))[0]
plt.plot(time_H[ind],Q_H[ind,1]*917./1e12,'r.',markersize=4, label = 'Flux out')
plt.ylabel('Ice flux \n'+r'(Gt yr$^{-1}$)',fontsize=9,fontname='Arial')
...
for i in range(0,len(yH_runoff)):
  ...
  patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
  ax.add_patch(patch)
plt.text(time1+0.2,35,'d',fontsize=9,fontname='Arial',fontweight='bold')
...
#plt.plot(time_H,(ubar_H[:,1]-ubar_H[:,0])/(dists_H[indbox_H[-1]]-dists_H[indbox_H[0]]),'b.',markersize=4)
#plt.ylabel(r'$\bar{u}_{in}-\bar{u}_{out}$'+'\n'+r'(km yr$^{-1}$)',fontsize=9,fontname='Arial')
plt.ylabel('Strain rate'+'\n'+r'(yr$^{-1}$)',fontsize=9,fontname='Arial')
...
for i in range(0,len(yH_runoff)):
  ...
  patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
  ax.add_patch(patch)
plt.text(time1+0.2,0.51,'b',fontsize=9,fontname='Arial',fontweight='bold')
...
plt.yticks(np.arange(0,0.011,0.001),fontsize=8,fontname='Arial')
...
for i in range(0,len(yH_runoff)):
  ...
  patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
  ax.add_patch(patch)
plt.text(time1+0.2,0.0095,'c',fontsize=9,fontname='Arial',fontweight='bold')
ax.set_xticklabels([])
...
plt.errorbar(dem_time_K[:,0],dem_dH_K[:,0],xerr=dem_time_K[:,1],yerr=dem_dH_K[:,1],fmt='ko',markersize=2.5,capsize=1,lw=0.5,label='DEM')
#plt.errorbar(time_K_2,dH_K_2,label='Helheim',fmt='r.',markersize=4,capsize=1,lw=0.5)
plt.errorbar(time_K,dH_K[:,0]+smb_K,fmt='ro',markersize=2.5,capsize=1,lw=0.5,label='Flux')
...
plt.plot(time_K[ind],Q_K[ind,0]*917./1e12,'b.',markersize=4,label = 'Flux in')
ind = np.where(~(np.isnan(Q_K[:,1])))[0]
plt.plot(time_K[ind],Q_K[ind,1]*917./1e12,'r.',markersize=4, label = 'Flux out')
plt.ylabel('Ice flux \n'+r'(Gt yr$^{-1}$)',fontsize=9,fontname='Arial')
plt.xticks(range(2008,2017))
plt.xlim([time1,time2])
plt.yticks(np.arange(20,40,4),fontsize=8,fontname='Arial')
...
for i in range(0,len(yH_runoff)):
  ...
  patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
  ax.add_patch(patch)
plt.text(time1+0.2,32.5,'d',fontsize=9,fontname='Arial',fontweight='bold')
plt.legend(loc=1,numpoints=1,ncol=3,handletextpad=0.2,fontsize=8,columnspacing=0.4,handlelength=0.3,borderpad=0.2)
labels=[]
for i in range(2000,2017):
  labels.append('Jan \n'+str(i))
...
plt.ylabel(r'Surface slope',fontsize=9,fontname='Arial')
plt.xticks(range(2008,2017))
...
for i in range(0,len(yH_runoff)):
  ...
  patch = matplotlib.patches.PathPatch(path,facecolor='0.85',edgecolor='none',lw=0)
  ax.add_patch(patch)
plt.text(time1+0.2,0.0227,'c',fontsize=9,fontname='Arial',fontweight='bold')
ax.set_xticklabels([])
