...
for i in (range(0,len(velocities_rkm[0,:]))):
  plt.plot(time_rkm,(velocities_rkm[:,i]-velocities_rkm[1,i])/1e3,'-',linewidth=2,color=coloptions[i],label=str(-dists[i])+' km')
ax.set_xticklabels([])
...
plt.legend(fontsize=18,ncol=2,handlelength=0.05)
...
for i in (range(0,len(velocities_rm[0,:]))):
  plt.plot(time_rm,(velocities_rm[:,i]-velocities_rm[1,i])/1e3,'-',linewidth=2,color=coloptions[i],label=str(-dists[i])+' km')
ax.set_xticklabels([])
plt.xticks(np.arange(0,60,10),fontsize=26)
plt.ylim([-0.5,2.1])
plt.yticks([0,1,2],fontsize=24)
...
ax.tick_params('both', length=10, width=1, which='major')
ax.tick_params('both', length=5, width=1, which='minor')
plt.legend(fontsize=17,ncol=2,handlelength=0.05)
...
velflowlines,times=helheim_velocity.velocity_along_flowline(flowline[:,1],flowline[:,2],flowline[:,0])
...
vel1s = np.zeros([len(timesteps),len(coord1s)])
for i in range(0,len(timesteps)):
  ind1 = np.where(surf_rkm['timestep'] == timesteps[i])
  coord1 = surf_rkm['coord1'][ind1]
  vel1 = surf_rkm['vel1'][ind1]
...
ax.tick_params('both', length=10, width=1, which='major')
ax.tick_params('both', length=5, width=1, which='minor')
plt.ylabel('           Velocity (km/yr)',fontsize=26)
...
plt.ylabel('Bed (m asl)',fontsize=26)
ax = plt.gca()
ax.tick_params('both', length=10, width=1, which='major')
ax.tick_params('both', length=5, width=1, which='minor')
