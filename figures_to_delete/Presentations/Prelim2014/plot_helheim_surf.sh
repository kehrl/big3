...
# Beds
SURF=$HOME/Data/DEMs/Gimp/gimpdem_helheim.txt
...
#xyz2grd $SURF -Gdem1.grd $R -I300 -NNaN
...
grdmath dem1.grd mask.grd MUL = dem2.grd


makecpt -Cgray -T0/255/1 -Z > grayscale.cpt
makecpt -Cglobe -T0/2000/50 -V -Z > colorscale.cpt

psbasemap $J $R $B -P -K -V > $FILE
grdimage $IMAGE -Cgray $J $R $B -K -O -V >> $FILE
grdimage dem2.grd -Ccolorscale.cpt $J $R $B -K -Q -O -V >> $FILE
grdcontour dem2.grd -C200 -W0.8p $J $R $B -K -Q -O -V >> $FILE
