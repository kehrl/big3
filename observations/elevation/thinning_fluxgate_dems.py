...
plt.errorbar(dem_time_H[:,0],dem_dH_H[:,0],xerr=dem_time_H[:,1],yerr=dem_dH_H[:,1],fmt='bo',markersize=3.5,capsize=1,lw=0.5,label='DEM')
#plt.errorbar(time_H_2,dH_H_2,label='Helheim',fmt='r.',markersize=4,capsize=1,lw=0.5)
plt.errorbar(time_H,dH_H[:,0]+smb_H,fmt='rs',markersize=3.5,capsize=1,lw=0.5,label='Flux-gate')
...
plt.errorbar(dem_time_K[:,0],dem_dH_K[:,0],xerr=dem_time_K[:,1],yerr=dem_dH_K[:,1],fmt='bo',markersize=3.5,capsize=1,lw=0.5,label='DEM')
#plt.errorbar(time_K_2,dH_K_2,label='Helheim',fmt='r.',markersize=4,capsize=1,lw=0.5)
plt.errorbar(time_K,dH_K[:,0]+smb_K,fmt='rs',markersize=3.5,capsize=1,lw=0.5,label='Flux')
...
plt.xlim([time1,time2])
plt.ylabel(r'dH/dt (m yr${-1}$)',fontname='Arial',fontsize=9)
plt.yticks(np.arange(-120,160,40),fontsize=8,fontname='Arial')
plt.ylim([-155,120])
ax.set_xticklabels([])
#xTickPos = np.linspace(np.floor(time1)-0.25,np.ceil(time2)-0.25,(np.ceil(time2)-np.floor(time1))*2+1)
#ax.bar(xTickPos, [max(plt.ylim())-min(plt.ylim())] * len(xTickPos), (xTickPos[1]-xTickPos[0]), bottom=min(plt.ylim()), color=['0.85','w'],linewidth=0)
