...
FUNCTION SeaPressure ( Model, nodenumber, y) RESULT(pw)
   ...
   IF ((FirstTime) .OR. (t > told)) THEN
     
      CALL INFO('SeaPressure','Memory allocation done.',level = 3)
 
      IF (.NOT. FirstTime) THEN
        DEALLOCATE( S, auxReal, Ns, a_perp, SourceFunc, NodeOnBoundary )
      END IF
...
FUNCTION SeaSpring ( Model, nodenumber, y) RESULT(C)
   ...
   IF ((FirstTime) .OR. (t > told)) THEN
      ...
      CALL INFO('SeaSpring','Memory allocation done.',level = 1)

      IF (.NOT. FirstTime) THEN
        DEALLOCATE( NodeOnBoundary, auxReal, Ns )
      END IF
