...
import argparse
import scipy
import datelib, elmerreadlib, glaclib, colorlib, geotifflib, meshlib
import cubehelix
...
x,y,test = geotifflib.read(DIR_FS_ModelT+'DEM'+dates[0]+'_bed_mod_taub.tif')
...
if i in range(0,nt):
  ...
  ax = plt.gca()
  plt.imshow(image[:,:,0],extent=[np.min(ximage),np.max(ximage),np.min(yimage),np.max(yimage)],cmap='Greys_r',origin='lower',clim=[0,0.6])
  p=plt.imshow(np.sqrt(ub_FS_ConstantT[:,:,i]**2+vb_FS_ConstantT[:,:,i]**2)/np.sqrt(us_FS_ConstantT[:,:,i]**2+vs_FS_ConstantT[:,:,i]**2),extent=[x[0],x[-1],y[0],y[-1]],origin='lower',vmin=0,vmax=1)
  plt.xticks([])
  plt.yticks([])
  plt.xlim([xmin,xmax])
  plt.ylim([ymin,ymax])
...
for k in range(0,len(bins)):
  for l in range(0,nt):
    ...
    taub_FS_ConstantT_bins[:,:,l,k] = scipy.ndimage.convolve(taub_FS_ConstantT[:,:,l],kernel,mode='constant',cval=np.float('nan'))  
    taub_FS_ModelT_bins[:,:,l,k] = scipy.ndimage.convolve(taub_FS_ModelT[:,:,l],kernel,mode='constant',cval=np.float('nan'))
    taub_SSA_ConstantT_bins[:,:,l,k] = scipy.ndimage.convolve(taub_SSA_ConstantT[:,:,l],kernel,mode='constant',cval=np.float('nan'))
    taub_SSA_ModelT_bins[:,:,l,k] = scipy.ndimage.convolve(taub_SSA_ModelT[:,:,l],kernel,mode='constant',cval=np.float('nan'))
    taud_bins[:,:,l,k] = scipy.ndimage.convolve(tauds_flow[:,:,l],kernel,mode='constant',cval=np.float('nan'))

for j in range(0,nt):
  nonnan = np.where(~(np.isnan(taub_FS_ConstantT_bins[:,:,0,-1])))
  for i in range(0,len(bins)):
    taub_FSCT_FSMT[j,i] = np.sqrt(np.mean(abs(taub_FS_ConstantT_bins[nonnan[0],nonnan[1],j,i]-taub_FS_ModelT_bins[nonnan[0],nonnan[1],j,i])**2)) 
    taub_FSCT_SSACT[j,i] = np.sqrt(np.mean(abs(taub_FS_ConstantT_bins[nonnan[0],nonnan[1],j,i]-taub_SSA_ConstantT_bins[nonnan[0],nonnan[1],j,i])**2))
    taub_FSCT_SSAMT[j,i] = np.sqrt(np.mean(abs(taub_FS_ConstantT_bins[nonnan[0],nonnan[1],j,i]-taub_SSA_ModelT_bins[nonnan[0],nonnan[1],j,i])**2))
    taub_FSMT_SSACT[j,i] = np.sqrt(np.mean(abs(taub_FS_ModelT_bins[nonnan[0],nonnan[1],j,i]-taub_SSA_ConstantT_bins[nonnan[0],nonnan[1],j,i])**2))
    taub_FSMT_SSAMT[j,i] = np.sqrt(np.mean(abs(taub_FS_ModelT_bins[nonnan[0],nonnan[1],j,i]-taub_SSA_ModelT_bins[nonnan[0],nonnan[1],j,i])**2))
    taub_SSACT_SSAMT[j,i] = np.sqrt(np.mean(abs(taub_SSA_ConstantT_bins[nonnan[0],nonnan[1],j,i]-taub_SSA_ModelT_bins[nonnan[0],nonnan[1],j,i])**2))
...
if regions == True:
  ...
  if glacier == 'Kanger':
    plt.legend(loc=4,fontsize=8,numpoints=1,handlelength=2.5,labelspacing=0.05,ncol=5,columnspacing=0.7,handletextpad=0.3,borderpad=0.25)
  ...
  if glacier == 'Helheim':
    plt.legend(loc=1,fontsize=8,numpoints=1,handlelength=2.5,labelspacing=0.05,ncol=5,columnspacing=0.7,handletextpad=0.3,borderpad=0.25)
